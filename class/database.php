...
class Database{
	...
	public function actionpie($action, $table1, $table2, $where){
		$sql = "{$action} {$where}, MAX(Rcount) + MAX(Acount) AS number FROM (SELECT {$where}, COUNT(*) AS Rcount, 0 AS Acount FROM {$table1} GROUP BY {$where} UNION ALL SELECT {$where}, 0, COUNT(*) FROM {$table2} GROUP BY {$where}) AS combine GROUP BY {$where} ORDER BY number DESC";
		if(!$this->queryall($sql)->error()){
			return $this;
		}
		return false;
	...
	public function getall2($table1, $table2, $where){
		return $this->actionpie('SELECT', $table1, $table2, $where);
	}

	public function actionleaderboard($action, $table, $where = array(), $where2){
		if(count($where) === 3){
			$operators = array('=','>','<','>=','<=');
			...
			if(in_array($operator, $operators)){
				$sql = "{$action} FROM {$table} WHERE {$field} {$operator} ? ORDER BY {$where2} DESC LIMIT 3";
	...
	// public function actionleaderboard($action, $table, $where=array(), $where2){
	// 	$sql = "{$action} FROM {$table} WHERE {$where2} ORDER BY {$where} DESC LIMIT 3";
	// 	if(!$this->queryall($sql)->error()){
	// 		return $this;
	// 	}
	// 	return false;
	...
	public function getall3($table, $where, $where2){
		return $this->actionleaderboard('SELECT *', $table, $where, $where2);
	}

	public function actiongiveallowance($action, $table, $where){
		$sql = "{$action} MONTH (CAST({$where} AS DATE)) AS month FROM {$table} WHERE description = 'Monthly Allowance'";
		if(!$this->queryall($sql)->error()){
			return $this;
		}
		return false;
	...
	public function getgiveallowance($table, $where){
		return $this->actiongiveallowance('SELECT', $table, $where);
	}

	// public function actionnull($action, $table, $where1=array(), $where2){
	// 	if(count($where1) === 3){
	// 		$operators = array('=','>','<','>=','<=');
	...
	// 			$sql = "{$action} FROM {$table} WHERE {$field1} {$operator1} ? AND {$where2} IS NULL";
	...
	// public function getnull($table, $where1, $where2){
	// 	return $this->actionnull('SELECT *', $table, $where1, $where2);
	// }
